...
const PostCard = ({ post }) => {
  const { title, slug, excerpt, image, category, author, createdAt } = post
  ...
  return (
    <article className="group flex flex-col overflow-hidden rounded-xl bg-white shadow-sm transition-shadow duration-300 hover:shadow-lg">
      ...
      <div className="flex flex-1 flex-col p-5">
        <Link to={`/post/${slug}`}>
          <h2 className="mb-2 text-lg font-bold text-gray-900 transition-colors duration-200 group-hover:text-blue-600 line-clamp-2">
            {title}
          </h2>
        </Link>
