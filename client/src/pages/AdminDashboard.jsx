...
const SkeletonRow = () => (
  <tr className="animate-pulse">
    <td className="px-4 py-3">
      <div className="h-10 w-16 rounded bg-gray-200 dark:bg-gray-700" />
    </td>
    <td className="px-4 py-3">
...
const SkeletonCard = () => (
  <div className="animate-pulse rounded-xl bg-white p-4 shadow-sm dark:bg-gray-800">
    <div className="flex gap-4">
      <div className="h-16 w-20 shrink-0 rounded-lg bg-gray-200 dark:bg-gray-700" />
  ...
  return (
    <div>
          ...
          <div className="rounded-xl bg-white p-5 shadow-sm dark:bg-gray-800">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900 dark:text-white">{uniqueCategories.length}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {loading && (
        <>
          {/* Desktop skeleton */}
          <div className="hidden overflow-hidden rounded-xl bg-white shadow-sm dark:bg-gray-800 md:block">
            <table className="w-full">
              <thead className="bg-gray-50 dark:bg-gray-800/80">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Image</th>
      ...
      {!loading && !error && posts.length === 0 && (
        <div className="rounded-xl bg-white py-16 text-center shadow-sm dark:bg-gray-800">
          <svg
          ...
          <h2 className="mb-2 text-xl font-semibold text-gray-700 dark:text-gray-200">No posts yet</h2>
          <p className="mb-6 text-gray-500 dark:text-gray-400">Create your first post to get started.</p>
          <Link
            to="/admin/posts/new"
      ...
      {!loading && posts.length > 0 && (
        <div className="hidden overflow-hidden rounded-xl bg-white shadow-sm dark:bg-gray-800 md:block">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {posts.map((post) => (
                <tr key={post._id} className="transition-colors hover:bg-gray-50 dark:hover:bg-gray-700/40">
                  <td className="px-4 py-3">
                    <img
                  ...
                  <td className="px-4 py-3">
                    <p className="max-w-xs truncate font-medium text-gray-900 dark:text-gray-100">{post.title}</p>
                  </td>
                  <td className="px-4 py-3">
                    <span className="inline-block rounded-full bg-blue-50 px-3 py-1 text-xs font-medium text-blue-700 dark:bg-blue-950/50 dark:text-blue-300">
                      {post.category}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <Link
                        ...
                        className="rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-medium text-gray-700 transition-colors hover:bg-gray-100 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-700"
                      >
                        Edit
                      </Link>
                      <button
      ...
      {!loading && posts.length > 0 && (
        <div className="space-y-3 md:hidden">
          {posts.map((post) => (
            <div key={post._id} className="rounded-xl bg-white p-4 shadow-sm dark:bg-gray-800">
              <div className="flex gap-4">
                <img
                  src={post.image || PLACEHOLDER_IMAGE}
                  alt={post.title}
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1 flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                    <span className="inline-block rounded-full bg-blue-50 px-2 py-0.5 text-xs font-medium text-blue-700 dark:bg-blue-950/50 dark:text-blue-300">
                      {post.category}
                    </span>
              ...
              <div className="mt-3 flex gap-2 border-t border-gray-100 pt-3 dark:border-gray-700">
                <Link
                  to={`/admin/posts/${post._id}`}
                  className="flex-1 rounded-lg border border-gray-200 py-2 text-center text-xs font-medium text-gray-700 transition-colors hover:bg-gray-100 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-700"
                >
                  Edit
                </Link>
                <button
                  onClick={() => handleDeleteClick(post)}
                  className="flex-1 rounded-lg border border-red-200 py-2 text-center text-xs font-medium text-red-600 transition-colors hover:bg-red-50 dark:border-red-900 dark:text-red-400 dark:hover:bg-red-950/40"
                >
                  Delete
                </button>
              </div>
